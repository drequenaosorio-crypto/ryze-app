  ...
  return (
    <main className="min-h-screen bg-black flex justify-center">
      <div className="relative w-full max-w-[400px] h-[100dvh] bg-black overflow-hidden">
        <video ref={videoRef} autoPlay loop playsInline muted={false} onClick={toggleAudio} className="absolute inset-0 w-full h-full object-cover">
        ...
        <div className="absolute right-3 bottom-24 flex flex-col items-center gap-6 z-10">
          ...
          <button onClick={() => alert("Comentarios pronto")} className="flex flex-col items-center">
            <div className="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center"><span className="text-xl">💬</span></div>
            <span className="text-white text-xs mt-1">348</span>
          </button>
          <button onClick={() => setSaved(!saved)} className="flex flex-col items-center">
            <div className={saved? "w-12 h-12 rounded-full bg-yellow-400 flex items-center justify-center" : "w-12 h-12 rounded-full bg-white/20 flex items-center justify-center"}><span className="text-xl">{saved? "🔖" : "📑"}</span></div>
          ...
          <button onClick={() => { navigator.clipboard.writeText("https://www.ryzeofficial-app.com"); alert("Link copiado"); }} className="flex flex-col items-center">
            <div className="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center"><span className="text-xl">↗️</span></div>
