      ...
      <div className="relative w-full max-w-[400px] h-[100dvh] bg-black overflow-hidden">
        ...
        <div className="absolute right-3 bottom-28 flex flex-col items-center gap-5 z-10">
          <button onClick={toggleLike} className="flex flex-col items-center">
            <div className={liked? "w-12 h-12 rounded-full bg-red-500 flex items-center justify-center" : "w-12 h-12 rounded-full bg-white/20 backdrop-blur flex items-center justify-center"}>
              <span className="text-xl">{liked? "❤️" : "🤍"}</span>
            </div>
            <span className="text-white text-xs font-bold mt-1">{likes >= 1000? (likes / 1000).toFixed(3) + "k" : likes}</span>
          </button>
